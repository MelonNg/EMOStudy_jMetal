...
public class MOEAD_IPBI_SBX extends Algorithm {
     ...
     */
    public MOEAD_IPBI_SBX(Problem problem) {
        super(problem);
...
//        z_ = new double[problem_.getNumberOfObjectives()];
        ...
        //weights_ = new Vector(problem_.getNumberOfObjectives()) ;
        ...
        initUniformWeight();
        //for (int i = 0; i < 300; i++)
        // 	System.out.println(weights_[i][0] + " " + weights_[i][1]) ;
...
//                updateReference(child);
     ...
     *
     */
    void updateNadirPoint() throws JMException, ClassNotFoundException {
        for (int i = 0; i < problem_.getNumberOfObjectives(); i++) {
            nz_[i] = -1.0e+30;
        } // for
        ...
        for (int i = 0; i < populationSize_; i++) {
            for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
                if (population_.get(i).getObjective(n) > nz_[n]) {
                    nz_[n] = population_.get(i).getObjective(n);
                }
            }
        } // for
    } // initIdealPoint
    ...
    double fitnessFunction(Solution individual, double[] lambda) {
        ...
        if (functionType_.equals("_TCHE1")) {
            ...
            for (int i = 0; i < problem_.getNumberOfObjectives(); i++)
                realA[i] = (individual.getObjective(i) - z_[i]);

            // distance along the line segment
            double d1 = innerProduct(realA, lambda);

            // distance to the line segment
            for (int i = 0; i < problem_.getNumberOfObjectives(); i++)
                realB[i] = (individual.getObjective(i) - (z_[i] + d1 * lambda[i]));
            double d2 = norm(realB);

            fitness = d1 + theta * d2;
        } else if (functionType_.equals("_IPBI")) {
            ...
            // difference between current point and reference point
            for (int n = 0; n < problem_.getNumberOfObjectives(); n++)
                realA[n] = (nz_[n] - individual.getObjective(n));
            ...
            // distance to the line segment
            for (int n = 0; n < problem_.getNumberOfObjectives(); n++)
                realB[n] = (nz_[n] - individual.getObjective(n) - d1
                        * lambda[n]);
            double d2 = norm(realB);
