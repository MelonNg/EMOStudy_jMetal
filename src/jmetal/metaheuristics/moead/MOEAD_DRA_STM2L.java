 ...
 */
public class MOEAD_DRA_STM2L extends Algorithm {
     ...
     */
    public MOEAD_DRA_STM2L(Problem problem) {
        super(problem);
...
//		return finalSelection(final_size);
    ...
    public int stableMatchingIncompletePreference(int[][] manPref, int[][] womanPref, int menSize, int womenSize, int[] statusMan, int[] statusWoman) {
        ...
        int unmatchedMenSize = 0;
        for (int i = 0; i < menSize; i++) {
            if (statusMan[i] == NOT_ENGAGED)
                unmatchedMenSize++;
        }
        return unmatchedMenSize;
    }

    private boolean containedInList(int x, int[] womanPref) {
        for (int i = 0; i < r_; i++) {
            if (womanPref[i] == x)
                return true;
        }
     ...
     * @return
     */
    private boolean prefers(int x, int y, int[] womanPref, int menSize) {
