 ...
 */
public class MOEAD_DRA_AMOSTM extends Algorithm {
    ...
    double delta_;
    /**
     * relatedSubp: solution's related subproblem
     */
    int[] relatedSubp_;

    LinkedList<Integer>[] subpStatus_;
     ...
     */
    public MOEAD_DRA_AMOSTM(Problem problem) {
        super(problem);
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        relatedSubp_ = new int[populationSize_];
        for (int i = 0; i < populationSize_; i++) {
            relatedSubp_[i] = i;
        }
        subpStatus_ = new LinkedList[populationSize_];
        for (int i = 0; i < populationSize_; i++) {
            subpStatus_[i] = new LinkedList<>();
            subpStatus_[i].add(i);
        }
...
//            solPreferListLengths[i] = 8;
        }

        CollegeAdmission cap = new CollegeAdmission(populationSize_, union_.size(), subpPref, solPref);
        subpStatus_ = new LinkedList[populationSize_];
        int[] solStatus = new int[union_.size()];
        cap.admitIncompleteLists(subpStatus_, solStatus, solPreferListLengths, populationSize_);

        int updatedSize = 0;
        for (int i = 0; i < populationSize_; i++) {
            for (int j = 0; j < subpStatus_[i].size(); j++) {
                population_.replace(updatedSize, new Solution(union_.get(subpStatus_[i].get(j))));
                subpStatus_[i].set(j, updatedSize);
                relatedSubp_[updatedSize] = i;
                updatedSize++;
            }
        }
    }
...
//		utility = theta * distance + (1 - theta) * scale;
    ...
    public void comp_utility() {
        ...
        for (int i = 0; i < populationSize_; i++) {
            ...
            if (firstSol != -1) {
                f1 = fitnessFunction(population_.get(firstSol), lambda_[i], functionType_);
                savedValues_[i] = new Solution(population_.get(firstSol));
            } else {
                f1 = f2;
            }
    ...
    public Solution[] matingSelection(Vector<Integer> list, int cid, int size, int type) {
        ...
        for (int j = 0; j < T_; j++) {
            neighbors.addAll(subpStatus_[neighborhood_[cid][j]]);
        }
        ss = neighbors.size();
//        if (ss <= T_/2) {
        if (ss < T_) {
...
//            selected.add(i);
        ...
        while (selected.size() < threshold) {
            ...
            for (int i = 1; i < depth; i++) {
                ...
                s2 = candidate.get(i2);
                if (utility_[relatedSubp_[s2]] > utility_[relatedSubp_[best_sub]]) {
                    best_idd = i2;
                    best_sub = s2;
                }
            }
