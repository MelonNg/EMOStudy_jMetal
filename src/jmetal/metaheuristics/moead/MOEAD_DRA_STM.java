...
public class MOEAD_DRA_STM extends Algorithm {
	...
	String dataDirectory_;
  	 ...
  	 */
	public MOEAD_DRA_STM(Problem problem) {
		super(problem);
...
//		idx = stableMatching2(solPref, subpPref, union_.size(), populationSize_);
	...
	public int[] stableMatching(int[][] manPref, int[][] womanPref, int menSize, int womenSize) {
  	...
  	/**
	 * Return the stable matching between 'subproblems' and 'solutions'
	 * ('solutions' propose first). It is worth noting that the number of
	 * solutions is larger than that of the subproblems.
	 * 
	 * @param manPref
	 * @param womanPref
	 ...
	 * @return
	 */
	public int[] stableMatching2(int[][] manPref, int[][] womanPref, int menSize, int womenSize) {
		...
		int matchedSize = menSize - freeMen.size();
		while (matchedSize < womenSize) {
			int m = freeMen.remove();
			int w = manPref[m][next[m]];
			next[m]++;
			if (statusWoman[w] == NOT_ENGAGED) {
			...
			matchedSize = menSize - freeMen.size();
		}
