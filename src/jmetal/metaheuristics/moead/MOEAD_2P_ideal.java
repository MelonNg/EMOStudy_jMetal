...
/**
 * This is a variant of MOEA/AD (two sets of subproblems are from the ideal point)
 */
public class MOEAD_2P_ideal extends Algorithm {
     ...
     */
    public MOEAD_2P_ideal(Problem problem) {
        super(problem);
    }
...
//            Utils.randomPermutation(permutation, populationSize_);
            ...
            String directory = "../experiments/MOEAD_2P_Study_PBI_TCHA_online/data/MOEAAD/" + problem_.getName() + "_" + problem_.getNumberOfObjectives();
            File experimentDirectory = new File(directory);
            if (!experimentDirectory.exists()) {
                boolean result = new File(directory).mkdirs();
                System.out.println("Creating " + directory);
            }
            if ((evaluations_-91)%(maxEvaluations_/40)==0) {
                population1_.printVariablesToFile(directory + "/pop_d." + evaluations_);
                population2_.printVariablesToFile(directory + "/pop_c." + evaluations_);
                population1_.printObjectivesToFile(directory + "/objs_d." + evaluations_);
                population2_.printObjectivesToFile(directory + "/objs_c." + evaluations_);
            }
